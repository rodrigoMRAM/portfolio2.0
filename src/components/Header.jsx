  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-sm border-b border-gray-200 ${darkMode && 'navbar-bg'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <nav className={`hidden md:flex space-x-8 ${darkMode ? 'text-white' : 'text-black'}`}>
            ...
            <button onClick={() => scrollToSection('projects')} id='projects_navbar' className=" hover:text-blue-600 transition-colors cursor-pointer">
              {t('navbar_projects')}
            </button>
      ...
      <button onClick={() => changeLanguage('es')}>ES</button> */}
            <label
              className={`relative bg-[#101828] inline-block w-17 h-7 rounded-full cursor-pointer transition-colors duration-300 bg-gradient-to-r from-[#9810fa] to-[#155dfc] ${checked ? 'bg-[#101828]' : 'bg-[#101828]'
                }`}
            >
              <input
                type="checkbox"
              ...
              {/* Texto ON/OFF */}
              <span
                className={`absolute left-2 top-1/2 transform -translate-y-1/2 text-xs font-bold transition-all duration-300 ${checked ? 'text-white left-2' : 'text-white left-12'
                  }`}
              >
                {checked ? 'EN' : 'ES'}
              </span>
              ...
              {/* Círculo */}
              <div
                className={`absolute top-1 left-1 w-5 h-5 bg-white rounded-full shadow-md transform transition-all duration-300 flex items-center justify-center overflow-hidden ${checked ? 'translate-x-10' : 'translate-x-0'
                  }`}
              >
                <img
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <nav ref={menuRef} className="md:hidden py-4 border-t border-gray-200">
